...
public class LevelDBService implements IStoreService {
    ...
    @Override
    public void put(long time,byte[] params){
        long[] key = {time,System.nanoTime()};// prevent repeat future time
        db.put(BinaryUtil.toBytes(key),params);
    }

    @Override
    public void putBatch(long[] times,byte[][] params){
        if(times.length==0){
            return;
        }
        ...
        for(int i=0;i<times.length;i++){
            ...
            batch.put(BinaryUtil.toBytes(key),params[i]);
        }
        db.write(batch);
        try {
            batch.close();
